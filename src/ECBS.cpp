...
/**
 * 综述论文中这样描述 EECBS：
 * 算法对 ECBS 进行了改进，它包含了显示估计搜索和三个启发式函数。
 * 第一个启发式函数是 A* 中的 h(n)，
 * 第二个启发式函数计算冲突的个数
 * 第三个是启发式函数是在线学习方法，在搜索过程中观察前面的 h(n) 和冲突个数的误差，然后反馈矫正。
 *
 * 此外 EECBS 还增加了近年来一些新的CBS改进技术。
 * 通过实验对比分析，CBS能解决智能体的个数小于 200，而 EECBS可以达到 1000 个以上，
 * 而且解的质量与最优解只有 2% 的误差。
 */
...
bool ECBS::solve(double time_limit, int _cost_lowerbound) {
    ...
    while (!cleanup_list.empty() && !solution_found) {
        ...
        if ((curr == dummy_start || curr->chosen_from == "cleanup") && !curr->h_computed) {
            ...
            // no solution, so prune this node
            if (!succ) {
                if (screen > 1)
        ...
        if (bypass && curr->chosen_from != "cleanup") {
            ...
            while (foundBypass) {
                    ...
                    if (!solved[i]) {
                        ...
                        for (const auto &path: child[i]->paths) {
                            ...
                            if ((double) path.second.first.size() - 1 >
                                suboptimality * fmin_copy[path.first]) {
                                foundBypass = false;
                                break;
                            }
                        }
                ...
                if (foundBypass) {
                    ...
                    // classify the new-detected conflicts
                    classifyConflicts(*curr);
                } else {
                    for (int i = 0; i < 2; i++) {
                        if (solved[i]) {
        ...
        if (!curr->children.empty())
            // update online heuristic errors
...
bool ECBS::generateRoot() {
    ...
    for (auto i: agents) {
        paths_found_initially[i] = search_engines[i]->findSuboptimalPath(*root, initial_constraints[i], paths, i, 0,
                                                                         suboptimality);
...
bool ECBS::findPathForSingleAgent(ECBSNode *node, int ag) {
    clock_t t = clock();
    auto new_path = search_engines[ag]->findSuboptimalPath(*node, initial_constraints[ag], paths, ag, min_f_vals[ag],
                                                           suboptimality);
    ...
    switch (solver_type) {
        case high_level_solver_type::EES:
            ...
            if (screen > 1 && cleanup_list.top()->getFVal() > cost_lowerbound)
                cout << "Lowerbound increases from " << cost_lowerbound << " to " << cleanup_list.top()->getFVal()
                     << endl;
            cost_lowerbound = max(cleanup_list.top()->getFVal(), cost_lowerbound);
            if (focal_list.top()->sum_of_costs <= suboptimality * cost_lowerbound) { // return best d
                curr = focal_list.top();
            ...
            } else if (open_list.top()->sum_of_costs <= suboptimality * cost_lowerbound) { // return best f_hat
                curr = open_list.top();
